...
export class NoteComponent implements OnInit {
  ...
  trackId = (_index: number, item: INote): string => this.noteService.getNoteIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.notes = this.refineData(dataFromBody);
  }

  protected refineData(data: INote[]): INote[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: INote[] | null): INote[] {
    return data ?? [];
  }
