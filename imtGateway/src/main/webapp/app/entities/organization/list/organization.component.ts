...
export class OrganizationComponent implements OnInit {
  ...
  trackId = (_index: number, item: IOrganization): string => this.organizationService.getOrganizationIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.organizations = this.refineData(dataFromBody);
  }

  protected refineData(data: IOrganization[]): IOrganization[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: IOrganization[] | null): IOrganization[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.organizationService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
