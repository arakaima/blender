...
/**
 * REST controller for managing {@link be.fgov.famhp.imt.gateway.domain.Site}.
 */
@RestController
@RequestMapping("/api")
public class SiteResource {
     ...
     */
    @PostMapping("/sites")
    public Mono<ResponseEntity<SiteDTO>> createSite(@RequestBody SiteDTO siteDTO) throws URISyntaxException {
     ...
     */
    @GetMapping("/sites")
    public Mono<List<SiteDTO>> getAllSites() {
        log.debug("REST request to get all Sites");
        return siteService.findAll().collectList();
    }

    /**
     * {@code GET  /sites} : get all the sites as a stream.
     * @return the {@link Flux} of sites.
     */
    @GetMapping(value = "/sites", produces = MediaType.APPLICATION_NDJSON_VALUE)
    public Flux<SiteDTO> getAllSitesAsStream() {
        log.debug("REST request to get all Sites as a stream");
     ...
     */
    @GetMapping("/sites/{id}")
    public Mono<ResponseEntity<SiteDTO>> getSite(@PathVariable String id) {
    ...
    public Mono<ResponseEntity<Void>> deleteSite(@PathVariable String id) {
