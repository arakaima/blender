...
class CapaResourceIT {
    ...
    void putExistingCapa() throws Exception {
        ...
        restCapaMockMvc
            .perform(
                put(ENTITY_API_URL_ID, capaDTO.getId())
                    .with(csrf())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(capaDTO))
            )
    ...
    void putWithMissingIdPathParamCapa() throws Exception {
        ...
        restCapaMockMvc
            .perform(
                put(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(capaDTO))
            )
    ...
    void patchNonExistingCapa() throws Exception {
        ...
        restCapaMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, capaDTO.getId())
                    .with(csrf())
                    .contentType("application/merge-patch+json")
                    .content(TestUtil.convertObjectToJsonBytes(capaDTO))
            )
