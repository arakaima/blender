...
class DeficiencyResourceIT {
        ...
        restDeficiencyMockMvc
            .perform(
                post(ENTITY_API_URL)
                    ...
                    .content(TestUtil.convertObjectToJsonBytes(deficiency))
            )
    ...
    void putExistingDeficiency() throws Exception {
        ...
        restDeficiencyMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedDeficiency.getId())
                    .with(csrf())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(updatedDeficiency))
            )
            .andExpect(status().isOk());
    ...
    void putNonExistingDeficiency() throws Exception {
        ...
        restDeficiencyMockMvc
            .perform(
                put(ENTITY_API_URL_ID, deficiency.getId())
                    .with(csrf())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(deficiency))
            )
    ...
    void patchNonExistingDeficiency() throws Exception {
        ...
        restDeficiencyMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, deficiency.getId())
                    .with(csrf())
                    .contentType("application/merge-patch+json")
                    .content(TestUtil.convertObjectToJsonBytes(deficiency))
            )
